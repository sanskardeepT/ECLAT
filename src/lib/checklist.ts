...
const CHECKLIST_RULES = [
  {
    ...
    weight: 20,
    tip: 'Start with a strong visual hook or a bold text overlay in the first 3 seconds',
  ...
  },
  {
    id: 7,
    name: 'Topic Tagged',
    description: 'Topic name is specified',
    weight: 10,
    tip: 'Always specify your topic — it helps the algorithm categorize your content.',
...
function generateCaption(input: ChecklistInput, score: number, items: ChecklistItem[]): string {
  const topic = input.dishTopic.trim() || 'Content'
  ...
  if (score >= 85) {
    ...
    hookLine = `Doing this with ${topic} is actually a game-changer 🚀`
  } else {
    hookLine = `Here's my take on ${topic} — tell me what you think! 👇`
  }

  // Value lines from passed checks
  const valueLines: string[] = []
  if (items.find(i => i.id === 5)?.passed) {
    valueLines.push('100% original content, no shortcuts')
  }
  ...
  if (valueLines.length === 0) {
    valueLines.push('Simple setup, amazing results')
  }
